...
class Preprocess:
    ...
    def __feature_engineering(self, df):
        ...
        def convert_time(s):
            ...
            return int(timestamp)
        df["Timestamp"] = df["Timestamp"].apply(lambda x: convert_time(str(x)))
        # TODO
        return df
...
class DKTDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        test, question, tag, correct, Tagrate, answerrate, elapsed, cumAnswerRate, Timestamp = \
            row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8]

        cate_cols = [test, question, tag, correct, Tagrate, answerrate, elapsed, cumAnswerRate, Timestamp]
